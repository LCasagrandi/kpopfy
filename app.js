...
const APIController = (function() {
    ...
    const clientId = '4671c4c3dd7d4cda86acbdae37ccfa00';
    const clientSecret = 'a302f5527b644ebbb2cd02a46fbb07d5';
...
const DOMElements = {
    ...
    divSongDetail: '#divSongDetail',
    hfToken: '#hfToken',
    divSongList: '#divSongList',
}
...
DOMInputs.genre.addEventListener('change', async () => {
    ...
    const genreSelect = UICtrl.inputField().genre; // Adicione os parênteses aqui
    const genreID = genreSelect.options[genreSelect.selectedIndex].value;
    const playlists = await APICtrl.getPlaylistsByGenre(token, genreID);
    playlists.forEach(p => UICtrl.createPlaylist(p.text, p.value)); // Passe os argumentos corretos
});
